<template>
  <div class="bg-white dark:bg-neutral-800 rounded-lg p-6 shadow-sm border border-gray-200 dark:border-gray-700">
      ...
      >
        <div class="flex justify-between items-start mb-3">
          <div class="flex flex-col">
            <div class="text-lg font-bold text-gray-900 dark:text-white">{{ token.symbol }}</div>
        ...
        <div class="mb-3">
          <div class="text-xl font-semibold" :class="getPriceChangeClass(token.change24h)">
            ${{ formatPrice(token.price) }}
          </div>
          <div class="text-sm font-medium" :class="getPriceChangeClass(token.change24h)">
    ...
    <!-- Loading state -->
    <div v-if="loading" class="flex items-center justify-center gap-2 text-gray-500 dark:text-gray-400 py-8">
        <div class="w-6 h-6 border-2 border-gray-300 border-t-blue-500 rounded-full animate-spin" />
      <span>Loading prices...</span>
    </div>

    <!-- Error state -->
    <div v-if="error" class="flex items-center justify-center gap-2 text-red-500 py-8">
      <span>Failed to load prices</span>
      <button class="px-4 py-2 text-sm bg-red-500 text-white rounded-md hover:bg-red-600 transition-colors" @click="refreshPrices">Retry</button>
    </div>
  </div>
</template>
...
const getTokenIcon = (symbol: string): string => {
  return tokenConfig[symbol as keyof typeof tokenConfig]?.icon || 'heroicons:currency-dollar'
}
