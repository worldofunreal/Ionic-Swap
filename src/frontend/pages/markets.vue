<template>
  <div class="h-full bg-gray-50 dark:bg-neutral-950 overflow-hidden">
    <div class="flex gap-4 p-6 h-full">
      ...
      <div class="w-80 flex">
        <div class="bg-white dark:bg-neutral-900 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 flex flex-col w-full h-full">
          <!-- Search Bar Header -->
          <div class="p-3 border-b border-gray-200 dark:border-gray-700 flex-shrink-0">
            ...
            <div v-if="isUsingMockData" class="mt-2">
              <div class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-yellow-100 dark:bg-yellow-900/20 text-yellow-800 dark:text-yellow-200">
          ...
          <div class="flex-1 overflow-y-auto">
            <div class="divide-y divide-gray-200 dark:divide-gray-700">
              <div
                ...
                :class="{ 'bg-blue-50 dark:bg-blue-900/20': selectedToken === token.symbol }"
                @click="selectToken(token.symbol)"
              >
                <div class="flex items-center justify-between">
                  <div class="flex items-center space-x-2">
                    <div class="w-6 h-6 rounded-full bg-gray-100 dark:bg-neutral-800 flex items-center justify-center">
                      <UIcon :name="getTokenIcon(token.symbol)" class="w-4 h-4" />
                    </div>
                    <div>
                      <div class="text-xs font-medium text-gray-900 dark:text-white">{{ token.symbol }}</div>
                      <div class="text-xs text-gray-500 dark:text-gray-400 truncate">{{ getTokenName(token.symbol) }}</div>
                    </div>
                  </div>
                  <div class="text-right">
                    <div class="text-xs font-medium text-gray-900 dark:text-white">${{ formatPrice(token.price) }}</div>
      ...
      <div class="flex-1 flex">
        <div v-if="selectedToken" class="bg-white dark:bg-neutral-900 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 flex flex-col w-full h-full">
