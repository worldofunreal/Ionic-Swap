  ...
  actions: {
    ...
    async login(walletType: WalletType) {
      try {
        ...
        // CRITICAL FIX: For local wallet, use the mnemonic directly to create identity
        // This ensures the identity matches the addresses already generated by LocalWalletAdapter
        if (walletType === 'local' && authResult.mnemonic) {
          // Local wallet: use mnemonic directly to ensure consistency
          const recovered = await CrossChainSeedService.fromMnemonic(authResult.mnemonic)
          identity = recovered.identity
          console.log('Using local wallet identity from mnemonic for consistency')
        } else {
          // Other wallets: use signature or principal
        ...
        if (existingProfile) {
          ...
          // Store mnemonic for local wallet
          if (walletType === 'local' && authResult.mnemonic) {
            sessionData.originalMnemonic = authResult.mnemonic
            console.log('- Original Mnemonic stored for local wallet')
          }
          ...
          // Save session so Settings can access recovery phrase immediately
          const sessionData: Record<string, unknown> = {
            authenticated: true,
            registered: false,
            ...
            nativeWallet: this.nativeWallet,
            canisterInitialized: true,
          ...
          // Store signature for wallets that have it, principal for Internet Identity
          if (walletType === 'internet-identity') {
            sessionData.originalPrincipal = authResult.principal
          } else {
            sessionData.originalSignature = authResult.signature
          }
    ...
    async completeRegistration(profile: User) {
      ...
      appCacheService.updateSession({ registered: true })
    ...
    async restoreSession() {
      ...
      if (session && session.authenticated) {
        ...
        try {
          ...
          if (session.originalSignature) {
          ...
          } else if (session.originalMnemonic && session.originalWalletType === 'local') {
            // For local wallet, recreate from stored mnemonic
            console.log('Recreating local wallet from stored mnemonic...')
            
            const recovered = await CrossChainSeedService.fromMnemonic(session.originalMnemonic)
            identity = recovered.identity
            ...
            if (profile) {
              ...
              console.log('Local wallet session restored successfully')
              return true
            } else {
              // User exists in session but not in database - needs registration
              console.log(
                'Local wallet user authenticated but not registered, keeping session for registration'
              )
              this.registered = false
              this.userProfile = null
              return true // Return true to keep the session
            ...
            console.warn(
              'No original signature, principal, or mnemonic available for session restoration'
            )
            appCacheService.clearSession()
            return false
          }
