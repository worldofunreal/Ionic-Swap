...
class PriceService {
  ...
  private updateCallbacks: Set<(prices: Map<string, TokenPrice>) => void> =
    new Set()
  ...
  async fetchInitialPrices(): Promise<void> {
    try {
      ...
      // Add USDT as a fixed $1.00 stablecoin (not available on Binance API)
      this.prices.set('USDT', {
        symbol: 'USDT',
        price: 1.0,
        change24h: 0.01, // Small positive change to show it's "stable"
        volume24h: 8000000000, // High volume typical for USDT
        timestamp: Date.now(),
      })
      ...
      console.log(
        'PriceService: Using fallback mock data due to API restrictions'
      )
      this.loadMockData()
    }
  }
  ...
  private loadMockData(): void {
    ...
    const mockPrices = [
      ...
      { symbol: 'USDT', price: 1.0, change24h: 0.01, volume24h: 8000000000 },
    ...
    console.log(
      'PriceService: Loaded mock data for',
      this.prices.size,
      'tokens'
    )
    this.notifySubscribers()
  ...
  private attemptReconnect(): void {
    if (this.reconnectAttempts < this.maxReconnectAttempts) {
      ...
      console.error(
        'Max reconnection attempts reached. Falling back to periodic REST API calls.'
      )
      this.startPeriodicFetch()
    }
  }
...
if (typeof window !== 'undefined') {
  ;(window as Record<string, unknown>).resetPriceService = () => {
