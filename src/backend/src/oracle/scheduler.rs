...
/// Start the price update scheduler (runs every 30 seconds)
pub async fn start_price_scheduler() -> Result<String, String> {
    ...
    ic_cdk::println!("🚀 Starting price update scheduler (30 second intervals)...");
    
    // Use IC's built-in timer system
    let timer_id = ic_cdk_timers::set_timer_interval(
        Duration::from_secs(30), // Every 30 seconds
        || {
    ...
    Ok("Price scheduler started successfully - updates every 30 seconds".to_string())
}
